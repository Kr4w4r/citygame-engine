...
Blender: 245
Group: 'Export'
Tooltip: 'Export to Quake2 file format (.omd2) -new-'
"""
...
__bpydoc__ = """\
This script exports a mesh object to the Quake2 "OMD2" file format.
...
Thanks to Bob Holcomb for OMD2_NORMALS taken from his exporter.<br>
Thanks to David Henry for the documentation about the OMD2 file format.
"""

#New Header Data
bl_info = {
	"name": "Export .omd2",
	"description": "Export to Quake2 file format (.omd2)",
	"author": "Dao Nguyen and Bernd Meyer, metaio GmbH (based on the initial exporter of Damien Thebault and Erwan Mathieu)",
	"version": (1, 0),
	"blender": (2, 5, 8),
	"api": 31236,
	"location": "File > Export > omd2",
...
OMD2_NORMALS=((-0.525731, 0.000000, 0.850651),
...
class OMD2:
	# groeßen der einzelnen Strukturen
	FILE_HEADER_SIZE = 20
	SKIN_SIZE = 68
	MESH_HEADER_SIZE = 40
	TEX_COORD_SIZE = 4
	TRIANGLE_SIZE = 26
	FRAME_SIZE = 40
	VERTEX_COMPRESSED_SIZE = 4
	GL_COMMAND_SIZE = 4
	GL_COMMAND_DATA_SIZE = 12

	def __init__(self, anim, basename): # Konstruktor
		self.anim = anim
	...
	def write(self, filename):
		...
		try:
			# Erstmal den Header zusammenbauen	
			self.fileHeader.ident = 'OMD2'
			self.fileHeader.version = 1
			
			# Anzahl der Skins ermitteln
			allSkins = set()
			# dazu von allen Meshes durchgehen und deren Skins in das Set einfügen,
			# dieses nimmt eine Instanz nicht doppelt auf
			for mesh in self.objectList:
			#mesh = self.objectList[0]
				skinList = Util.getSkins(mesh)
				for skin in skinList:
					allSkins.append(skin)
			# Anzahl der Skins abspeichern
			self.fileHeader.numSkins = len(allSkins)
			
			# Anzahl der Meshes abspeichern
			self.fileHeader.numMeshes = len(self.objectList)
			
			# und noch die Offsets für den FileHeader ermitteln
			self.fileHeader.offsetSkins = FILE_HEADER_SIZE
			self.fileHeader.offsetMeshes = self.fileHeader.offsetSkins + self.fileHeader.numSkins * SKIN_SIZE
			
			# dann können wir schonmal den FileHeader in die Datei schreiben
			bin = struct.pack('<1s1L2H2L', 
							  self.fileHeader.ident,
							  self.fileHeader.version,
							  self.fileHeader.numSkins,
							  self.fileHeader.numMeshes,
							  self.fileHeader.offsetSkins,
							  self.fileHeader.offsetMeshes)
			file.write(bin)
	...
	def outFrame(self, file, min, max, frameName = 'frame'):
		...
		for vert in mesh.vertices:
			for i in range(162):
				dot =  vert.normal[1]*OMD2_NORMALS[i][0] + \
					  -vert.normal[0]*OMD2_NORMALS[i][1] + \
					   vert.normal[2]*OMD2_NORMALS[i][2]
...
class Util:
	@staticmethod
	def pickName():
		name = '_OMD2Obj_'+str(random.random())
		return name[0:20]
	...
	def __init__(self, object):
		...
		if self.ismesh:
			...
			try:
				...
				self.status = (str(len(mesh.vertices)) + ' vertices', str(len(mesh.faces)) + ' faces')
...
class Export_OMD2(bpy.types.Operator, ExportHelper):
	"""Export to Quake2 file format (.omd2)"""
	bl_idname = "export_quake.omd2"
	bl_label = "Export to Quake2 file format (.omd2)"
	...
	def execute(self, context):
		...
		if len(self.objectList[:]) == 0:
			raise NameError('Please, select one Mesh!')
		...
		self.exportList = []
		for mesh in self.objectList:
			info = ObjectInfo(mesh)
			
			if info.triang == True:
				tmpObjectName = Util.pickName()
				duplicatedObject = Util.duplicateObject(mesh, tmpObjectName)
				tempMeshList.append(duplicatesObject)
				exportList.append(triangulateMesh(duplicatedObject))
			else:
				exportList.append(mesh)
		...
		try:
			omd2 = OMD2(self.val_anim, self.val_basename)
			omd2.setObjects(exportList)
			omd2.write(filepath) #omd2.write(filename)
		finally:
			if object.name == tmpObjectName:
				originalObject.select = True
				bpy.context.scene.objects.active = originalObject
				#Util.deleteObject(object);
				for tempMesh in tempMeshList:
					Util.deleteObject(tempMesh)
...
def menuCB(self, context):
	self.layout.operator(Export_OMD2.bl_idname, text="Export to OMD2...")
 
def register():
	# initialize()
	# bpy.utils.register_class(Export_OMD2)
